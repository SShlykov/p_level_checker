## Введение
Расширяя команду мы ищем человека с которым будем готовы работать длительный промежуток времени. Поэтому выставляем выставляем следующие требования к соискателю и, исходя из выполненых задач, оцениваем уровень разработчика.
...
* Уверенно владеет библиотеками (одним или несколькими фреймворками) JS на уровне, который позволяет решать абсолютное большинство стандартных задач, связанных с интерфейсами.
* Умеет ясно излагать свои мысли в письменной и устной форме, понимает важность проактивности для командной работы.

### Middle frontend developer
* Отлично знает инструментарий, включая работу с LiveView и событиями.
* Имеет навыки проектирования и знает основные принципы разработки поддерживаемого продукта, т.е. владеет ООП, ФП, понимает DDD, SOLID, Event Driven архитектура.
* Умеет спроектировать небольшого и среднего размера стандартные проекты (интерфейс для проекта до 200k строк с учетом команды).
* Умеет эффективно (качественно и в срок) решать задачи в рамках заданной архитектуры.
* Знает, как решить нетривиальные локальные задачи, чаще всего не связанные с интерфейсами напрямую.
...
* Может помочь ясно сформулировать требования к задаче.
* Может спроектировать верхнеуровневую архитектуру продукта учитывая весь контекст (пользователи, бизнес, 3rd party services).
...
`При составлении была использована карта программиста от MetaLamp`
[карта](https://github.com/fullstack-development/developers-roadmap.git)